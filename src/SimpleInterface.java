...
public class SimpleInterface extends JFrame implements KeyListener{
	...
	private LinkedList<Integer> lastKey;
	private LinkedList<MyMouseEvent> lastClicked;
	 ...
	 */
	public void clear(){
		clearRect(0,0,img.getWidth(),img.getHeight());
	}
	...
	public int flushKey(){
		if (!hasKey())
			return -1;
		int k = lastKey.getLast();
		lastKey.clear();
		return k;
	}
	
	public int[] flushClick(){
		if (!hasClick())
			return null;
		MyMouseEvent e = lastClicked.getLast();
		lastClicked.clear();
		return new int[]{e.x,e.y,e.but};
	}
